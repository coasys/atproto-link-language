/**
 * Local link store — wraps the StorageAdapter KV API to maintain
 ...
 *   revision                         → last known cursor/revision
 *   at-records/{rkey}                → AT URI of the record
...
/**
 * Check if a link exists by its hash.
 */
export function hasLink(linkHash: string): boolean {
    return getStorage().get(linkKey(linkHash)) !== null;
}
...
/**
 * Return the count of links in the store.
 */
export function linkCount(): number {
    return getStorage().listKeys("links/").length;
}
...
// ---------------------------------------------------------------------------
// AT record tracking (rkey → AT URI)
// ---------------------------------------------------------------------------

export function putRecordUri(rkey: string, uri: string): void {
    getStorage().put(`at-records/${rkey}`, uri);
}

export function getRecordUri(rkey: string): string | null {
    return getStorage().get(`at-records/${rkey}`);
}

export function removeRecordUri(rkey: string): void {
    getStorage().delete(`at-records/${rkey}`);
}
